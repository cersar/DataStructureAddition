// 约瑟夫斯排列，rank-RBTree法(m不为常数时，O(nlg(n)))
...
void printNode(TreeNode *n) {
	if (n != NULL) {
		printf("[key=%d,value=%f,color=%d,size=%d]\n", n->key, n->val, n->color, n->size);
	}
}
...
void updateSize(TreeNode *x) {
	while (x != NULL) {
		x->size = x->size = size(x->left) + size(x->right) + 1;
		x = x->p;
	}
}
...
void JosephusPermutation(RBTree *t,int start,int m) {
	TreeNode *out = NULL;
	int r = start;
	while (*(t) != NULL) {
		r = (r - 2 + m) % (*t)->size + 1;
